...
namespace ss
{
	...
	GameLib::Sprite* texture[TEXTURE_MAX];				//セルマップの参照するテクスチャ情報の保持
		...
		while (exit)
		{
			if (texture[texture_index] == 0)	//使われていないテクスチャ情報
			{
				...
				else
				{
					...
					wchar_t ws[100];
					mbstowcs(ws, pszFileName, 100);
					texture[texture_index] = GameLib::sprite_load(ws, 128);
...
//			glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_ALPHA, GL_SRC_ALPHA);
	...
	*/
	void SSDrawMesh(CustomSprite *sprite, State state)
	{
		...
		GameLib::draw_triangles_indexed(texture[tex_index], 
			reinterpret_cast<VECTOR3*>(sprite->_mesh_vertices), 
			reinterpret_cast<VECTOR4*>(sprite->_mesh_colors), 
			reinterpret_cast<VECTOR2*>(sprite->_mesh_uvs), 
			reinterpret_cast<short*>(sprite->_mesh_indices),
			sprite->_meshVertexSize, 
			sprite->_meshTriangleSize, 
			true);
...
//		if (texture[tex_index]->texture_is_pow2 == true)
		...
		if (_ssDrawState.texture != texture[tex_index])
		{
		//	glEnable(gl_target);
		//	//テクスチャのバインド
		//	glBindTexture(gl_target, texture[tex_index]);
		}
		...
		if (_ssDrawState.partBlendfunc != state.blendfunc)
		{
			...
			else
			{
				...
				case BLEND_MUL:		///< 1 乗算
					...
					GameLib::setBlendMode(GameLib::Blender::BS_MULTIPLY);
#if OPENGLES20
				...
				case BLEND_SUB:		///< 3 減算
					...
					GameLib::setBlendMode(GameLib::Blender::BS_SUBTRACT);
				...
				case BLEND_SCREEN:		///< 5 スクリーン
					...
					GameLib::setBlendMode(GameLib::Blender::BS_SCREEN);
		...
		if (sprite->_partData.type == PARTTYPE_MESH)
		{
			...
			_ssDrawState.texture = texture[tex_index];
		...
		if ((state.flags & PART_FLAG_PARTS_COLOR) || (state.flags & PART_FLAG_VERTEX_TRANSFORM))
		{
			...
			static short indices[] = { 4,3,1,0,2,3 };

			
			GameLib::draw_triangle_fan_indexed(texture[tex_index],
				reinterpret_cast<VECTOR3*>(vertices),
				reinterpret_cast<VECTOR4*>(colors),
				reinterpret_cast<VECTOR2*>(uvs),
				indices,
				5,
				4,
				true);
		...
		else
		{
			...
			for (i = 0; i < 4; i++)
			{
				_color[i].x = colors[i * 4];
				_color[i].y = colors[i * 4 + 1];
				_color[i].z = colors[i * 4 + 2];
				_color[i].w = colors[i * 4 + 3];
			...
			GameLib::draw_triangle_strip(texture[tex_index],
				reinterpret_cast<VECTOR3*>(vertices),
				reinterpret_cast<VECTOR4*>(colors),
				reinterpret_cast<VECTOR2*>(uvs),
				5,
