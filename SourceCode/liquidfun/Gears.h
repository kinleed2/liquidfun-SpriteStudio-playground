...
public:
	...
	void Step(Settings* settings)
	{
		...
		m_debugDraw.DrawString(5, m_textLine, "theta1 + %4.2f * theta2 = %4.2f", static_cast<float>(ratio), static_cast<float>(value));
		m_textLine += DRAW_STRING_NEW_LINE;

		ratio = m_joint5->GetRatio();
		value = m_joint2->GetJointAngle() + ratio * m_joint3->GetJointTranslation();
		m_debugDraw.DrawString(5, m_textLine, "theta2 + %4.2f * delta = %4.2f", static_cast<float>(ratio), static_cast<float>(value));
		m_textLine += DRAW_STRING_NEW_LINE;
	}
